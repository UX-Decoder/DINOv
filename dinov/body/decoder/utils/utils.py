...
def from_divisablity(x, div):
    if x % div == 0:
        return x
    return (int(x / div) + 1) * div

def getIdx(a, id_start):
    co = a.unsqueeze(0) - a.unsqueeze(1)
    uniquer = co.unique(dim=0)
    out = []
    for r in uniquer:
        cover = torch.arange(a.size(0)).to(a)
        mask = r == 0
        idx = cover[mask]
        out.append(idx)
    out = [o + id_start for o in out]
    return {str(k.cpu().numpy()): v for k, v in zip(a.unique(), out[::-1])}
...
def get_unpadded_tensor(tensors, num_examples):
    new_tensor_list = []
    for i, tensor in enumerate(tensors):
        new_tensor_list.append(tensor[:num_examples[i]])
